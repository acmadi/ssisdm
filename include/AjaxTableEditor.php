...
class AjaxTableEditor
{
	...
	function addRow()
	{
		if(stristr($this->permissions,'A'))
		{
			...
			$html .= '</table></form>
			<div id="addRowButtons">
			<button class="ajaxButton" onclick="'.$jsAddFun.'">'.$this->langVars->btnAdd.'</button><button class="ajaxButton" onclick="toAjaxTableEditor(\'update_html\',\'\');">'.$this->langVars->btnCancel.'</button>
			</div>';
	...
	function getAdvancedSearchHtml()
	{
		...
		$html .= '
		<p align="center">
		<p>'. $this->langVars->infoactiveclass.' '.$this->langVars->lblMatch.'
