...
if(isSet($_GET['peg_id2'])) 
	{
	$peg_id2 = $_GET['peg_id2'];
	}else
	{
...
if ($_POST['submit']<>"")
 {      
		...
		if($result) 
		{   
		echo "<META HTTP-EQUIV = 'Refresh' Content = '1; URL =employee_master_kontrakulang.php'>";
...
function cekkota()
{
  var peg_id2 = document.formData.peg_id2.value;
  
  $.ajax({
    type:"POST",
    url:"get_kota.php",    
  data: 'peg_id2=' + peg_id2,          
    success: function (html){                 
...
function cek() 
      {
	var perpanjang_kontrak = document.formData.perpanjang_kontrak.value;
    if (perpanjang_kontrak.length == 0) {
		alert("Keputusan perpanjang kontrak tidak boleh kosong");
		document.formData.perpanjang_kontrak.focus();
		return false;
   	}
...
<?php
	...
	if($peg_id){
	   ...
	   $query = mysql_query('SELECT * FROM view_emp_pegawai WHERE peg_id = "'.$peg_id.'" ');
	   if($query && mysql_num_rows($query) == 1){
	      $data = mysql_fetch_object($query);
			...
			$peg_id2=@$data->peg_id2;
	   }else 
	      die('Tidak Ada data');
	}?>


<form  name="formData" action="employee_master_kontrakulangaction.php" method="post" onSubmit="return cek();" enctype="multipart/form-data">
<fieldset>
<input type="hidden" name="peg_id" id="peg_id" value="<?php echo $peg_id;?>">

<input type="hidden" name="peg_id2" id="peg_id2" value="<?php echo $peg_id2;?>">
...
<tr><td width="24%">Perpanjang Kontrak?? </td><td>
<select name="perpanjang" id="perpanjang"  >
<option value="">[Pilih]</option>				
										<option value="Y"<?php if (!(strcmp(Y, $perpanjang))) {echo "SELECTED";} ?>>Y</option>
										<option value="N"<?php if (!(strcmp(N, $perpanjang))) {echo "SELECTED";} ?>>N</option>
									</select>
