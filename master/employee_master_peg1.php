...
if ($_POST['submit']<>"")
 {      
		...
		$npp= $_POST['npp'];	
		$lokasi_id2= $_POST['lokasi_id2'];	
		$unit_bisnis_id= $_POST['unit_bisnis_id'];	
		$pangkat_id= $_POST['pangkat_id'];	
		$narasi_kontrak= $_POST['narasi_kontrak'];	
		$periode_kontrak_trx= $_POST['periode_kontrak_trx'];	
		$rollover= $_POST['rollover'];	
	                    $tahun=date('Y');
						$query_cek = "SELECT nokontrak_SK FROM peg_dinas WHERE year(tgl_awal) = '$tahun' order by nokontrak_SK desc limit 1 ";
						...
						elseif ($nokontrak<1000)
							   ...
							   {$nokontrak=$nokontrak;}
	                            ...
	                            $nokontrak="SDM".$tahun.$nokontrak;
	                                          $query_cek = "SELECT kontrak_ke FROM peg_dinas WHERE peg_id = '$peg_id' order by kontrak_ke desc limit 1 ";
											 $resultno=mysql_query($query_cek) or (mysql_error()) ;
											$row_no = mysql_fetch_assoc($resultno);
		...
		$sql1 .= "lokasi_id2='$lokasi_id2',"; 
		$sql1 .= "unit_bisnis_id='$unit_bisnis_id',";
		$sql1 .= "pangkat_id='$pangkat_id',";
		$sql1 .= "tgl_awal='$tgl_awal',";
		$sql1 .= "tgl_akhir='$tgl_akhir',";
		$sql1 .= "narasi_kontrak='$narasi_kontrak', ";
		$sql1 .= "periode_kontrak_trx='$periode_kontrak_trx',";
		$sql1 .= "nokontrak_SK='$nokontrak',"; 
		$sql1 .= "kontrak_ke='$kontrak_ke',";  
		$sql1 .= "rollover='$rollover' ";  

		$result = mysql_query($sql1) ;
		                 
				       $sql = "Update  peg_personal set  ";
                       $sql .= "status_kerja_id='5' ";
...
cekorg();
ceksakit();
$('#ambiltgkakhir').click(function()
	{
	  
		var tgl_awal = $('#tgl_awal').val(); 
		var periode_kontrak = $('#periode_kontrak_trx').val(); 
	
	

     if (tgl_awal.length == 0) {
					alert("Anda harus memasukan tgl awal  !");
					document.formData.tgl_awal.focus();
					return false;
		}
		  if (periode_kontrak.length == 0) {
					alert("Anda harus memasukan periode_kontrak !");
		...
		data: "tgl_awal=" + tgl_awal+ "&periode_kontrak=" + periode_kontrak,         
     ...
     $('#lokasi_id').change(function() 
	 {
        ...
        $.ajax({
				type:"POST",
				url:"getlokasidetail2.php",    
				data: "lokasi_id=" + lokasi_id ,        
				success: function (html) {                 
				  $('#getlokasidetail2').html(html);
                                                   }  
                      });
     });	
function ceksakit()
...
function cek() 
      {
	var periode_kontrak_trx = document.formData.periode_kontrak_trx.value;
    if (periode_kontrak_trx.length == 0) {
		alert("periode kontrak tidak boleh kosong");
		document.formData.periode_kontrak_trx.focus();
		return false;
   	}
   ...
   var npp = document.formData.npp.value;
	...
	{
	       var lokasi_id2 = document.formData.lokasi_id2.value;
	       if (lokasi_id2.length == 0) {
			alert("lokasi  tidak boleh kosong");
			document.formData.lokasi_id2.focus();
			return false;
				}
	}
...
<form  name="formData" action="employee_master_peg1.php" method="post" onSubmit="return cek();" enctype="multipart/form-data">
<fieldset>
...
<div id="tabs-8">
<table border="1" width="60%" cellpadding="1" cellspacing="1" align="center">
<tr><td >1.) Kapabilitas terhadap Pekerjaan,Kompetensi,Kesesuaian terhadap jabatan</td> <td > <?php echo @$data->kapabilitas_pekerjaan;?></td></tr>
<tr><td >2.) Hubungan dengan Pekerjaan,Kompetensi dengan pekerjaan,pengalaman Perbankkan/Adm/tek</td> <td > <?php echo @$data->kompetensi;?></td></tr>
<tr><td >3.) Kemampuan terhadap pekerjaan (-kecukupan,-jabatan/posisi yg sesuai)</td> <td ><?php echo @$data->kemampuan;?></td></tr>
<tr><td >4.) Kapabilitas Pimpinan (-Perencanaan,-Sikap dan prilaku,-Pengawasan,-Motivasi)</td> <td > <?php echo @$data->kapabilitas_pimpinan;?></td></tr>
<tr><td colspan="2">5.) Karakteristik</td> </td></tr>
<tr><td >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Penampilan</td> <td > <?php echo @$data->penampilan;?></td></tr>
<tr><td >&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Kominikasi</td> <td > <?php echo @$data->komunikasi;?></td></tr>
<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Inisiatif</td> <td > <?php echo @$data->inisiatif;?></td></tr>
<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Kepemimpinan</td> <td > <?php echo @$data->kepemimpinan;?></td></tr>
<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Penalaran</td> <td > <?php echo @$data->penalaran;?></td></tr>
<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Semangat Kerja</td> <td > <?php echo @$data->semangatkerja;?></td></tr>
<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Lain-lain</td> <td > <?php echo @$data->lainlain;?></td></tr>
<tr><td colspan="2">6.) Rekomendasi</td> </td></tr>
<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Keputusan</td> <td > <?php $keputusan_interview=@$data->keputusan_interview;?>
...
<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Melanjukan Test</td> <td ><?php $lanjuttest=@$data->lanjuttest;?>
...
<tr><td>Alasan</td> <td > <?php echo @$data->alasan;?></td></tr>
		<tr>
        	<td>Tanggal  Interview</td>
            <td > 
			<?php echo datedatesql(@$data->tgl_interview);?>
...
?>
<select name="periode_kontrak_trx" id="periode_kontrak_trx"  >
<option value="">[Pilih]</option>
        <?php 		
...
<option value="">[Pilih]</option>				
	 ...
	 <tr>
		...
		<tr>
			...
			<td>
            	<select name="lokasi_id" id="lokasi_id" onKeyPress="return focusNext('tgl_awal',event)" >
                      <option value="">Pilihan</option>
					<?php
		  	         $query_a="select * FROM lokasi_kerja    where  aktif=1  and level=1 order by lokasi_kerja" ;
		  	         $result_a=mysql_query($query_a) or (mysql_error()) ;
						while($row_a=mysql_fetch_array($result_a)) 	{?>
					<option value="<?php echo $row_a['lokasi'];?>" 						
											<?php if (!(strcmp($row_a['lokasi'], @$data->lokasi))) {echo "SELECTED";} ?>>
											<?php echo $row_a['lokasi_kerja']; ?></option>
					<?php  
					   }
					?>
...
<tr>
	...
	</script><input type="button" name="ambiltgkakhir" id="ambiltgkakhir" value="Cari Tanggal Akhir Kontrak" >
...
<tr>
        	<td>Keterangan</td> 
            <td > <textarea name="narasikontrak" id="narasikontrak" cols="40" rows="3"><?php echo @$data->narasikontrak;?></textarea></td>
        </tr>
<tr>
    <td colspan="2">   
...
<?php
		$view = mysql_query("SELECT * FROM view_peg_dinashistori where
                         peg_id	='$peg_id'
					   order by kontrak_ke
						");
						...
						{ ?><tr>
							   ...
							   <td align="left"><?php echo $data['kontrak_ke'] ?></td>
								<td align="left"><?php echo datedatesql($data['tgl_awal']) ?></td>
							   	<td align="left"><?php echo datedatesql($data['tgl_akhir']) ?></td></tr>
		<?php } ?>	
</div>	
